#%% LSTM model predictions with the usage of Traffic Signals in the intersection as Model features
import pandas as pd
import numpy as np
...
numHorizon = 30
framePerSecond = 10
# reading the base data and removing vehicles not in our interest
...
# creating one-hot encoded vector for traffic signals
df["trafficSignal"].replace("y", "ye", inplace=True)
dff = df["trafficSignal"].copy()
...
# %% focusing on the vehicles used the intersection
validVehicles = []
...
for vehID, df in groupData:
    ...
    inputData = []
    outputData = []
    curVehData = df.loc[:, ["time", "x", "y", "speed", "angle", 'G', 'GG', 'Gg', 'Gr', 'g', 'o',
                            'r', 'rG', 'rr', 'ry', 'ye', 'yg', 'yr', 'yy']] # features of our interest
...
for i in range(len(training_X)):
    trainingData_X.append(training_X[i].reshape(int(len(training_X[i]) / numLagsPoints), numLagsPoints, 18))
    trainingData_Y.append(training_Y[i].reshape(int(len(training_Y[i]) / numHorizon), numHorizon, 2))

validationData_X = []
validationData_Y = []
for i in range(len(validation_X)):
    validationData_X.append(validation_X[i].reshape(int(len(validation_X[i]) / numLagsPoints), numLagsPoints, 18))
    validationData_Y.append(validation_Y[i].reshape(int(len(validation_Y[i]) / numHorizon), numHorizon, 2))

testingData_X = []
testingData_Y = []
for i in range(len(testing_X)):
    testingData_X.append(testing_X[i].reshape(int(len(testing_X[i]) / numLagsPoints), numLagsPoints, 18))
...
for i in range(len(straight_testing_X)):
    straight_testingData_X.append(
        straight_testing_X[i].reshape(int(len(straight_testing_X[i]) / numLagsPoints), numLagsPoints, 18))
...
for i in range(len(turning_testing_X)):
    turning_testingData_X.append(
        turning_testing_X[i].reshape(int(len(turning_testing_X[i]) / numLagsPoints), numLagsPoints, 18))
...
encoderModels["{}-{}-{}".format(hiddenLayer, learningRate, batchSize)].save(
    "./savedModels/encoder_SUMOData_TLS_3-3_{}-{}-{}.h5".format(hiddenLayer, learningRate, batchSize))
# %%
...
for mdl in list(encoderModels.keys()):
    ...
    pd.DataFrame.from_dict(eucledianDist, orient="index").to_csv(
        "euclDistOutput_TLS_3-3_{}.csv".format(mdl))

# %% calculating the mean euclidean distance for all non-turning testing vehicles
...
for mdl in list(predictedModelsStraightEncoder.keys()):
    ...
    pd.DataFrame.from_dict(eucledianDist_straight, orient="index").to_csv(
        "euclDistOutputStraight_TLS_3-3_{}.csv".format(mdl))

# %% calculating the mean euclidean distance for all turning testing vehicles
...
for mdl in list(predictedModelsTurnedEncoder.keys()):
    ...
    pd.DataFrame.from_dict(eucledianDist_turned, orient="index").to_csv(
        "euclDistOutputTurned_TLS_3-3_{}.csv".format(mdl))
