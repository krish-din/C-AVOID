...
dumm.rename(columns={0: 'Lane0', 1: 'Lane1', 2: 'Lane2', 3: 'Lane3'}, inplace=True)
...
for i in collidingVehicles.itertuples():
    ...
    if i.collisionVehicleID not in collisionPairs[i.veh_id]:
        collisionPairs[i.veh_id].append(i.collisionVehicleID)
...
collidedVehicles = list(collisionPairs.keys())
...
for i in x:
    if i not in collisionTrainingVehicles and i not in collisionValidationVehicles:
        ...
        collisionTestingVehicles.extend(collisionPairs[i])
...
if not saved:
    ...
    for i in range(len(training_X)):
        trainingData_X.append(training_X[i].reshape(int(len(training_X[i]) / numLagsPoints), numLagsPoints, 12))
        trainingData_Y.append(training_Y[i].reshape(int(len(training_Y[i]) / numHorizon), numHorizon, 2))

    validationData_X = []
    validationData_Y = []
    for i in range(len(validation_X)):
        validationData_X.append(validation_X[i].reshape(int(len(validation_X[i]) / numLagsPoints), numLagsPoints, 12))
        validationData_Y.append(validation_Y[i].reshape(int(len(validation_Y[i]) / numHorizon), numHorizon, 2))

    testingData_X = []
    testingData_Y = []
    for i in range(len(testing_X)):
        testingData_X.append(testing_X[i].reshape(int(len(testing_X[i]) / numLagsPoints), numLagsPoints, 12))
    ...
    for i in range(len(straight_testing_X)):
        straight_testingData_X.append(
            straight_testing_X[i].reshape(int(len(straight_testing_X[i]) / numLagsPoints), numLagsPoints, 12))
    ...
    for i in range(len(turning_testing_X)):
        turning_testingData_X.append(
            turning_testing_X[i].reshape(int(len(turning_testing_X[i]) / numLagsPoints), numLagsPoints, 12))
    ...
    df_train_x = pd.DataFrame(trainingData_X.reshape(int(len(trainingData_X) * numLagsPoints), 12))
    df_train_x.to_csv("trainingDataX_Unregulated_wULIPad_wACC_3-3.csv")
    del df_train_x
    df_train_y = pd.DataFrame(trainingData_Y.reshape(int(len(trainingData_Y) * numHorizon), 2))
    df_train_y.to_csv("trainingDataY_Unregulated_wULIPad_wACC_3-3.csv")
    del df_train_y
    df_val_x = pd.DataFrame(validationData_X.reshape(int(len(validationData_X) * numLagsPoints), 12))
    df_val_x.to_csv("valDataX_Unregulated_wULIPad_wACC_3-3.csv")
    del df_val_x
    df_val_y = pd.DataFrame(validationData_Y.reshape(int(len(validationData_Y) * numHorizon), 2))
    df_val_y.to_csv("valDataY_Unregulated_wULIPad_wACC_3-3.csv")
    del df_val_y
    df_test_x = pd.DataFrame(testingData_X.reshape(int(len(testingData_X) * numLagsPoints), 12))
    df_test_x.to_csv("testingDataX_Unregulated_wULIPad_wACC_3-3.csv")
    del df_test_x
    df_test_y = pd.DataFrame(testingData_Y.reshape(int(len(testingData_Y) * numHorizon), 2))
    df_test_y.to_csv("testingDataY_Unregulated_wULIPad_wACC_3-3.csv")
    del df_test_y

    df_test_x_stgt = pd.DataFrame(straight_testingData_X.reshape(int(len(straight_testingData_X) * numLagsPoints), 12))
    df_test_x_stgt.to_csv("testingDataXSTGHT_Unregulated_wULIPad_wACC_3-3.csv")
    del df_test_x_stgt
    df_test_y_stght = pd.DataFrame(straight_testingData_Y.reshape(int(len(straight_testingData_Y) * numHorizon), 2))
    df_test_y_stght.to_csv("testingDataYSTGHT_Unregulated_wULIPad_wACC_3-3.csv")
    del df_test_y_stght

    df_test_x_turn = pd.DataFrame(turning_testingData_X.reshape(int(len(turning_testingData_X) * numLagsPoints), 12))
    df_test_x_turn.to_csv("testingDataXTURN_Unregulated_wULIPad_wACC_3-3.csv")
    del df_test_x_turn
    df_test_y_turn = pd.DataFrame(turning_testingData_Y.reshape(int(len(turning_testingData_Y) * numHorizon), 2))
    df_test_y_turn.to_csv("testingDataYTURN_Unregulated_wULIPad_wACC_3-3.csv")
    del df_test_y_turn
    ...
    df = pd.read_csv("trainingDataX_Unregulated_wULIPad_wACC_3-3.csv")
    df.drop("Unnamed: 0", inplace=True, axis=1)
    trainingData = np.array(df)
    trainingData_X = trainingData.reshape(int(len(trainingData) / numLagsPoints), 30, 12)

    df = pd.read_csv("trainingDataY_Unregulated_wULIPad_wACC_3-3.csv")
    df.drop("Unnamed: 0", inplace=True, axis=1)
    trainingData = np.array(df)
    trainingData_Y = trainingData.reshape(int(len(trainingData) / numHorizon), 30, 2)

    df = pd.read_csv("valDataX_Unregulated_wULIPad_wACC_3-3.csv")
    df.drop("Unnamed: 0", inplace=True, axis=1)
    validationData = np.array(df)
    validationData_X = validationData.reshape(int(len(validationData) / numLagsPoints), 30, 12)

    df = pd.read_csv("valDataY_Unregulated_wULIPad_wACC_3-3.csv")
    df.drop("Unnamed: 0", inplace=True, axis=1)
    validationData = np.array(df)
    validationData_Y = validationData.reshape(int(len(validationData) / numHorizon), 30, 2)

    df = pd.read_csv("testingDataX_Unregulated_wULIPad_wACC_3-3.csv")
    df.drop("Unnamed: 0", inplace=True, axis=1)
    testingData = np.array(df)
    testingData_X = testingData.reshape(int(len(testingData) / numLagsPoints), 30, 12)

    df = pd.read_csv("testingDataY_Unregulated_wULIPad_wACC_3-3.csv")
    df.drop("Unnamed: 0", inplace=True, axis=1)
    testingData = np.array(df)
    testingData_Y = testingData.reshape(int(len(testingData) / numHorizon), 30, 2)

    df = pd.read_csv("testingDataXSTGHT_Unregulated_wULIPad_wACC_3-3.csv")
    df.drop("Unnamed: 0", inplace=True, axis=1)
    testingData = np.array(df)
    straight_testingData_X = testingData.reshape(int(len(testingData) / numLagsPoints), 30, 12)

    df = pd.read_csv("testingDataYSTGHT_Unregulated_wULIPad_wACC_3-3.csv")
    df.drop("Unnamed: 0", inplace=True, axis=1)
    testingData = np.array(df)
    straight_testingData_Y = testingData.reshape(int(len(testingData) / numHorizon), 30, 2)

    df = pd.read_csv("testingDataXTURN_Unregulated_wULIPad_wACC_3-3.csv")
    df.drop("Unnamed: 0", inplace=True, axis=1)
    testingData = np.array(df)
    turning_testingData_X = testingData.reshape(int(len(testingData) / numLagsPoints), 30, 12)

    df = pd.read_csv("testingDataYTURN_Unregulated_wULIPad_wACC_3-3.csv")
    df.drop("Unnamed: 0", inplace=True, axis=1)
    testingData = np.array(df)
...
learningRate = [0.0001]
batchSize = [32, 48, 64]
hiddenLayers = [[128, 128], [256, 128]]
for hiddenLayer in hiddenLayers:
    for lr in learningRate:
        for bs in batchSize:
            # if (hiddenLayer == [256, 128] and bs in [64]) or \
            #         (hiddenLayer == [128, 128] and bs in [32]):
            #     continue
            print("current_setupMultipleSUMO_Unregulated_wULIPad_wACC_{}-{}-{}".format(hiddenLayer, lr, bs))
            ...
            encoderModels["{}-{}-{}".format(hiddenLayer, lr, bs)].save_weights \
                ('./checkpoints/my_checkpoint_encoder_SUMOData_Unregulated_wULIPad_wACC_3-3_{}-{}-{}'.format(
                    hiddenLayer, lr, bs))
            encoderModels["{}-{}-{}".format(hiddenLayer, lr, bs)].save(
                "./savedModels/encoder_SUMOData_Unregulated_wULIPad_wACC_3-3_{}-{}-{}.h5".format(hiddenLayer, lr,
                                                                                                    bs))
# %%
...
for mdl in list(encoderModels.keys()):
    ...
    pd.DataFrame.from_dict(rmseEncoder, orient="index").to_csv(
        "rmseOutput_Unregulated_wULIPad_wACC_3-3_{}.csv".format(mdl))
    pd.DataFrame.from_dict(eucledianDist, orient="index").to_csv(
        "euclDistOutput_Unregulated_wULIPad_wACC_3-3_{}.csv".format(mdl))
...
for mdl in list(predictedModelsStraightEncoder.keys()):
    ...
    pd.DataFrame.from_dict(rmseEncoder_straight, orient="index").to_csv(
        "rmseOutputStraight_Unregulated_wULIPad_wACC_3-3_{}.csv".format(mdl))
    pd.DataFrame.from_dict(eucledianDist_straight, orient="index").to_csv(
        "euclDistOutputStraight_Unregulated_wULIPad_wACC_3-3_{}.csv".format(mdl))
...
for mdl in list(predictedModelsTurnedEncoder.keys()):
    ...
    pd.DataFrame.from_dict(rmseEncoder_turned, orient="index").to_csv(
        "rmseOutputTurned_Unregulated_wULIPad_wACC_3-3_{}.csv".format(mdl))
    pd.DataFrame.from_dict(eucledianDist_turned, orient="index").to_csv(
        "euclDistOutputTurned_Unregulated_wULIPad_wACC_3-3_{}.csv".format(mdl))
