...
df1 = pd.read_csv("orph_b1_tracksAvgTLS_reduced.csv")
maxVeh = df1.veh_id.max()+1
df2 = pd.read_csv("orph_b2_tracksAvgTLS_reduced.csv")
df2["veh_id"] = df2["veh_id"]+maxVeh
maxVeh = df2.veh_id.max()+1
df3 = pd.read_csv("orph_b3_tracksAvgTLS_reduced.csv")
df3["veh_id"] = df3["veh_id"]+maxVeh
maxVeh = df3.veh_id.max()+1
df4 = pd.read_csv("orph_b4_tracksAvgTLS_reduced.csv")
df4["veh_id"] = df4["veh_id"]+maxVeh
df = pd.concat([df1, df2, df3, df4])
del df1, df2, df3, df4
# creating one-hot encoded vector for traffic lights
dff = df["TLS"].copy()
...
def prepareData(data, n_lags, n_seq, initial):
    ...
    data.loc[:, ['CenterPointX', 'CenterPointY', 'Velocity', 'TTPC']] = \
        np.divide(np.subtract(data.loc[:, ['CenterPointX', 'CenterPointY', 'Velocity', 'TTPC']],
...
for vehId in uniqueVehicles:
    curVehData = df_Filtered[df_Filtered["veh_id"] == vehId][
        ["CenterPointX", "CenterPointY"]]  # , "HeadingAngle", "SpeedX", "SpeedY"]
...
#%% filtering the non-valid vehicles (vehicles didn't use the intersection)
...
#%%
allData = df_Filtered[df_Filtered["veh_id"].isin(trainingVehicles)][['CenterPointX',
                                                                     'CenterPointY', 'Velocity', 'TTPC']]
# standardisation
...
for vehID, df in groupData:
    ...
    inputData = []
    outputData = []
    curVehData = df.loc[:, ["frame", 'CenterPointX', 'CenterPointY', 'Velocity', 'TTPC',
...
for i in range(len(training_X)):
    trainingData_X.append(training_X[i].reshape(int(len(training_X[i]) / numLagsPoints), numLagsPoints, 9))
    trainingData_Y.append(training_Y[i].reshape(int(len(training_Y[i]) / numHorizon), numHorizon, 2))

validationData_X = []
validationData_Y = []
for i in range(len(validation_X)):
    validationData_X.append(validation_X[i].reshape(int(len(validation_X[i]) / numLagsPoints), numLagsPoints, 9))
    validationData_Y.append(validation_Y[i].reshape(int(len(validation_Y[i]) / numHorizon), numHorizon, 2))

testingData_X = []
testingData_Y = []
for i in range(len(testing_X)):
    testingData_X.append(testing_X[i].reshape(int(len(testing_X[i]) / numLagsPoints), numLagsPoints, 9))
...
for i in range(len(straight_testing_X)):
    straight_testingData_X.append(
        straight_testing_X[i].reshape(int(len(straight_testing_X[i]) / numLagsPoints), numLagsPoints, 9))
...
for i in range(len(turning_testing_X)):
    turning_testingData_X.append(
        turning_testing_X[i].reshape(int(len(turning_testing_X[i]) / numLagsPoints), numLagsPoints, 9))
...
encoderModels["{}-{}-{}".format(hiddenLayer, learningRate, batchSize)].save(
    "./savedModels/encoder_UAVData_TLS_3-3_{}-{}-{}.h5".format(hiddenLayer, learningRate, batchSize))
...
for mdl in list(encoderModels.keys()):
    ...
    for j in range(int(numHorizon / framePerSecond)):
        ...
        del a, b, c
    pd.DataFrame.from_dict(eucledianDist, orient="index").\
        to_csv("euclDistOutput_UAVData_TLS_3-3_{}.csv".format(mdl))
...
for mdl in list(predictedModelsStraightEncoder.keys()):
    ...
    pd.DataFrame.from_dict(eucledianDist_straight, orient="index").to_csv(
        "euclDistOutputStraight_UAVData_TLS_3-3_{}.csv".format(mdl))
...
for mdl in list(predictedModelsTurnedEncoder.keys()):
    ...
    pd.DataFrame.from_dict(eucledianDist_turned, orient="index").to_csv(
        "euclDistOutputTurned_UAVData_TLS_3-3_{}.csv".format(mdl))
